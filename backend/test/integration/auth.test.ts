...
describe('Auth routes', () => {
    ...
    describe('POST /v1/auth/logout', () => {
        test('should return 204 if refresh token is valid', async () => {
            ...
            const expires = add(new Date(), { days: config.jwt.refreshExpirationDays });
    ...
    describe('POST /v1/auth/refresh-tokens', () => {
        ...
        test('should return 401 error if refresh token is expired', async () => {
            ...
            const expires = sub(new Date(), { minutes: 1 });
            const refreshToken = tokenService.generateToken(
                dbUserOne.id,
                expires,
                TokenType.REFRESH
    ...
    describe('POST /v1/auth/send-verification-email', () => {
        ...
        test('should return 204 and send verification email to the user', async () => {
            ...
            const userOneAccessToken = tokenService.generateToken(
                dbUserOne.id,
                add(new Date(), { minutes: config.jwt.accessExpirationMinutes }),
                TokenType.ACCESS
            );
...
describe('Auth middleware', () => {
    ...
    test('should call next with unauthorized error if the token is not an access token', async () => {
        ...
        const expires = add(new Date(), { minutes: config.jwt.accessExpirationMinutes });
        const refreshToken = tokenService.generateToken(dbUserOne.id, expires, TokenType.REFRESH);
        const req = httpMocks.createRequest({
